...
def editar_informacoes(request):
    ...
    form = RestauranteEditForm(
        request.POST or None, request.FILES or None, instance=restaurante)
...
def info_home(request):
    ...
    context = {
        ...
        'current_url': current_url_name(request),
    }
...
def listar_pedidos(request):
    ...
    pedidos = Pedido.objects.filter(restaurante=restaurante).order_by(
        '-data_hora')  # Ordena pelos mais recentes
...
def cadastrar_pedido(request):
    restaurante = get_object_or_404(Restaurante, user=request.user)
    # Não tem upload de arquivo para Pedido
...
def editar_pedido(request, id):
    ...
    if pedido.restaurante != restaurante:
        messages.error(
            request, 'Você não tem permissão para editar este pedido.')
        return redirect('listar_pedidos')
...
def remover_pedido(request, id):
    ...
    if pedido.restaurante != restaurante:
        messages.error(
            request, 'Você não tem permissão para remover este pedido.')
        return redirect('listar_pedidos')
...
def cadastrar_item_cardapio(request):
    ...
    return render(request, 'cardapio/form.html', context)
...
def editar_item_cardapio(request, id):
    ...
    if item_cardapio.restaurante != restaurante:
        messages.error(
            request, 'Você não tem permissão para editar este item.')
        return redirect('listar_itens_cardapio')
...
def remover_item_cardapio(request, id):
    ...
    if item_cardapio.restaurante != restaurante:
        messages.error(
            request, 'Você não tem permissão para remover este item.')
        return redirect('listar_itens_cardapio')
